...
public class Control {
	...
	MyFile f = new MyFile("Chain Supermarket\\src\\persistence\\listcategorydat.txt");
	MyFile j = new MyFile("Chain Supermarket\\src\\persistence\\categories.txt");
	MyFile k = new MyFile("Chain Supermarket\\src\\persistence\\bills.txt");
	MyFile h = new MyFile("Chain Supermarket\\src\\persistence\\products.txt");
	...
	private void showAllSale() {
		MyFile j = new MyFile("Chain Supermarket\\src\\persistence\\items.txt");
		this.readbills();
		this.writeItems(j);
	...
	private void addSupllier() {
		try {
			MyFile j = new MyFile("Chain Supermarket\\src\\persistence\\suppliers.txt");
			short rut = io.readGraphicShort("Digite el Rut del Proveedor");
			if (sql.findSupplier(rut) == -1) {
				Supplier s = new Supplier(rut, io.readGraphicString("Digite un nombre"),
						io.readGraphicString("Digite un número"),
	...
	private void addClient() {
		MyFile j = new MyFile("Chain Supermarket\\src\\persistence\\clients.txt");
	...
	private void addProduct() {
		MyFile s = new MyFile("Chain Supermarket\\src\\persistence\\bills.txt");
		MyFile j = new MyFile("Chain Supermarket\\src\\persistence\\products.txt");
		short rut = io.readGraphicShort("Digite el Rut del proveedor");
		if (sql.findSupplier(rut) != -1) {
	...
	private void insertCategory(int id, short rut) {
		MyFile c = new MyFile("Chain Supermarket\\src\\persistence\\categories.txt");
			...
			while ((cad = f.readRecord()) != null) {
				...
				if (sql.findCategory(c.getId()) == -1) {
					...
					while (Integer.parseInt(dataCa[3]) != count) {
						sql.getListCategory().get(sql.findCategory(c.getId())).getListProducts()
								.add(this.splitProduct(dataCa[5 + count]));
						sql.getListSuplliers().get(sql.findSupplier(Short.parseShort(dataCa[4])))
								.addProduct(this.splitProduct(dataCa[5 + count]));
						;
						count++;
					}
